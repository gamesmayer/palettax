...
export function ColorDialog({ paletteId, color, colorSystem, onClose }: ColorDialogProps): JSX.Element {
  ...
  function handleSubmit(): void {
    const changes = { r: rgb.r, g: rgb.g, b: rgb.b, hex: rgbToHex(rgb.r, rgb.g, rgb.b), name: name.trim() ? name.trim() : undefined };
    if (color) {
  ...
  return (
    <div className="dialog-backdrop" onMouseDown={handleBackdropMouseDown}>
      ...
      >
        <Modal.Content>
          <ColorSystemFields colorSystem={colorSystem} rgb={rgb} onChange={setRgb} />
