...
import { ColorSystem, generateShadesAndTints, rgbToHex } from '../../../../shared/color';
...
export function ShadeTintDialog({ paletteId, colors, colorSystem, onClose }: ShadeTintDialogProps): JSX.Element {
  const addColors = usePaletteStore((state) => state.addColors);
  const insertColorsAroundId = usePaletteStore((state) => state.insertColorsAroundId);

  const [mode, setMode] = useState<EndpointMode>(colors.length > 0 ? 'palette' : 'new');
  const [paletteColorId, setPaletteColorId] = useState(colors[0]?.id ?? '');
  const [customRgb, setCustomRgb] = useState<Rgb>(colors[0] ?? { r: 255, g: 255, b: 255 });
  ...
  const base = mode === 'palette' ? colors.find((color) => color.id === paletteColorId) ?? customRgb : customRgb;
  const { shades, tints } = generateShadesAndTints(base, shadeCount, tintCount, lightnessStep);
  ...
  function handleSubmit(): void {
    ...
    const toStored = ({ r, g, b }: Rgb) => ({ r, g, b, hex: rgbToHex(r, g, b) });
          ...
          <div className="shade-tint-dialog__counts">
            <div className="endpoint-picker__field">
              ...
              <Input
                ...
                max={MAX_COUNT}
                value={shadeCount}
                onChange={(event) => setShadeCount(clampCount(Number(event.target.value)))}
            ...
            <div className="endpoint-picker__field">
              ...
              <Input
                ...
                max={MAX_COUNT}
                value={tintCount}
                onChange={(event) => setTintCount(clampCount(Number(event.target.value)))}
                aria-label="Number of tints"
              />
            </div>
            <div className="endpoint-picker__field">
              <span className="endpoint-picker__field-label">Lightness step %</span>
              <Input
                type="number"
                min={MIN_STEP}
                max={MAX_STEP}
                value={lightnessStep}
                onChange={(event) => setLightnessStep(clampStep(Number(event.target.value)))}
