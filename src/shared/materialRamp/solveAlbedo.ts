 ...
 * inverse: evaluateNeutralBaseColor composes sRGB<->linear conversion, the
 * metallic f0 lerp, the diffuse scale, the ambient/environment Fresnel
 * terms, and Reinhard tonemapping, all of which are per-channel-independent
 * pointwise operations with no cross-channel coupling anywhere -- so each
 * output channel is a monotonically non-decreasing function of ONLY the
 * matching input channel, for fixed metallic/roughness/lighting (verified in
 * brdf.test.ts). That guarantees bisection converges to the input byte
 * producing the closest achievable output byte, without needing to invert
 * the composed formula symbolically. The other two candidate channels
 * passed to evaluateNeutralBaseColor while solving this one are
 * mathematically irrelevant -- filled with 0 rather than a "real" guess so
 * that's visually obvious at each call site.
 */
...
/**
 * Back-solves the albedo (material.baseColor) that makes
 * evaluateNeutralBaseColor produce `target`, for the given metallic,
 * roughness, and lighting. `achieved` is evaluateNeutralBaseColor
 * re-evaluated at the final (rounded) albedo -- callers use it to detect an
