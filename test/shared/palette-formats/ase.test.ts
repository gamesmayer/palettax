import { parseAse, serializeAse } from '../../../src/shared/palette-formats/ase';
...
function colorEntryBlock(name: string, model: string, components: number[]): number[] {
  const body = [
    ...nameBytes(name),
...
describe('parseAse', () => {
  ...
  it('approximates an RGB color from a Lab color entry', () => {
    const content = buildAse([colorEntryBlock('', 'LAB ', [53.24, 80.09, 67.2])]);
    const palette = parseAse(content, 'test.ase');
  ...
  it('round-trips a palette with a named group and an ungrouped color', () => {
    const original: Palette = {
      ...
      groups: [
        ...
        {
          id: 'g2',
          colors: [{ id: 'c3', r: 1, g: 254, b: 127, hex: '#01fe7f', name: 'Odd' }]
        }
      ]
    };
    ...
    expect(roundTripped.groups[1].colors.map(({ r, g, b, name }) => ({ r, g, b, name }))).toEqual([
      { r: 1, g: 254, b: 127, name: 'Odd' }
    ]);
  });
});
