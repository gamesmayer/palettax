import { parseAco, serializeAco } from '../../../src/shared/palette-formats/aco';
...
function rawEntry(colorSpace: number, w1: number, w2: number, w3: number, w4: number): number[] {
  return [...u16(colorSpace), ...u16(w1 & 0xffff), ...u16(w2 & 0xffff), ...u16(w3 & 0xffff), ...u16(w4 & 0xffff)];
}

function v1Entry(colorSpace: number, w1: number, w2: number, w3: number, w4: number): number[] {
...
function buildAcoV1PlusV2(v1Entries: number[][], v2Entries: number[][]): string {
  const bytes = [
    ...u16(1),
...
describe('parseAco', () => {
  ...
  it('decodes negative Lab a/b components as signed 16-bit values', () => {
    const entryBytes = [...u16(7), ...u16(5000), ...i16(-5000), ...i16(-5000), ...u16(0)];
  ...
  it('converts a Grayscale color entry', () => {
    ...
    const palette = parseAco(content, 'test.aco');
    expect(palette.groups[0].colors[0]).toMatchObject({ r: 128, g: 128, b: 128 });
  });
  ...
  it('throws PaletteParseError for a buffer shorter than the minimum header', () => {
    expect(() => parseAco(fromBytes([0, 1]), 'test.aco')).toThrow(PaletteParseError);
  });
  ...
  it('round-trips a palette across groups, flattening into a single group', () => {
    const original: Palette = {
      ...
      groups: [
        ...
        {
          id: 'g2',
          colors: [{ id: 'c3', r: 128, g: 64, b: 32, hex: '#804020', name: 'Brown' }]
        }
      ]
    };
    ...
    expect(
      roundTripped.groups[0].colors.map(({ r, g, b, name }) => ({ r, g, b, name }))
    ).toEqual([
