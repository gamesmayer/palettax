...
function color(id: string, overrides: Partial<PaletteColor> = {}): PaletteColor {
  return { id, r: 0, g: 0, b: 0, hex: '#000000', ...overrides };
}

function group(id: string, colors: PaletteColor[], name?: string): PaletteGroup {
  return { id, name, colors };
}

describe('flattenGroups', () => {
  it('concatenates every group colors in order', () => {
    const groups = [group('g1', [color('a'), color('b')]), group('g2', [color('c')])];
...
describe('addColorToGroup', () => {
  it('appends the color to the matching group only', () => {
    ...
    const result = addColorToGroup(groups, 'g2', { r: 1, g: 2, b: 3, hex: '#010203' });
...
describe('removeColorFromGroup', () => {
  it('removes only the targeted color from the targeted group', () => {
    const groups = [group('g1', [color('a'), color('b')]), group('g2', [color('a')])];
...
describe('updateColorInGroup', () => {
  it('merges the given changes into the matching color', () => {
    ...
    const result = updateColorInGroup(groups, 'g1', 'a', { r: 9, hex: '#090000' });
