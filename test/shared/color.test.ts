...
describe('oklchToRgb', () => {
  ...
  it('brings an out-of-gamut chroma back into range while preserving hue', () => {
    ...
    const hueDiff = Math.min(Math.abs(h - requestedHue), 360 - Math.abs(h - requestedHue));
...
describe('generateShadesAndTints', () => {
  ...
  function expectHueClose(actual: number, expected: number, tolerance = 1.5): void {
    const diff = Math.abs(actual - expected);
    expect(Math.min(diff, 360 - diff)).toBeLessThan(tolerance);
  }

  function expectChromaClose(actual: number, expected: number, tolerance = 0.08): void {
  ...
  it('works when a shift object is missing individual fields (defaults each to 0)', () => {
    const { shades } = generateShadesAndTints(base, 1, 0, { lightnessShift: 40 });
  ...
  it('wraps hue around 360/0 instead of overshooting', () => {
    ...
    const { shades } = generateShadesAndTints(fabricatedBase, 1, 0, { hueShift: 20 });
  ...
  it('interpolates chroma progressively for a positive chroma shift', () => {
    ...
    const { tints } = generateShadesAndTints(base, 0, 4, {}, { chromaShift: 20 });
  ...
  it('interpolates tints toward the configured lighter target', () => {
    const { tints } = generateShadesAndTints(base, 0, 1, {}, { lightnessShift: 30 });
  ...
  it('behaves deterministically for an achromatic base color', () => {
    ...
    const second = generateShadesAndTints(grayBase, 3, 3, { ...shift }, { ...shift });
  ...
  it('always generates identical RGB output for identical inputs', () => {
    ...
    const first = generateShadesAndTints(base, 3, 2, { ...shift }, { ...shift });
    const second = generateShadesAndTints(base, 3, 2, { ...shift }, { ...shift });
    expect(first).toEqual(second);
  });
});
...
describe('generateShadesAndTints with easing', () => {
  ...
  it('defaults to linear when no easing argument is given', () => {
    const withoutArg = generateShadesAndTints(base, 4, 0, { lightnessShift: 40 });
  ...
  it.each(EASING_METHODS)(
    ...
    (easing) => {
      ...
      const targetC = Math.max(0, baseOklch.c + (shift.chromaShift / 100) * 0.4);
